    ...
    class Structure
    {
        ...
        function __destruct()
        {
            ...
            foreach ( [ 'primary', 'unique', 'index' ] as $type )
            {
                if ( $struct[ $type ] )
                {
                    switch ( $type )
                    {
                        case 'primary':
                            if ( !$struct[ 'increment' ] )
                                Builder::add_index( 'primary', $struct[ 'name' ] );
                            break;

                        default:
                            Builder::add_index( $type, $struct[ 'name' ] );
                            break;
                    }
                }
            }
            ...
            if ( $this->starting_index )
                Builder::add_extension( "AUTO_INCREMENT = $this->starting_index" );

            # Insert Column
            Builder::add_column( trim( $this->sql ) );
        }
    }